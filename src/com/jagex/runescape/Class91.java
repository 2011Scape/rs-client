...
public class Class91
{
	...
	static final void method1032(int i, Player player, int i_0_, Packet packet, int i_1_) {
		...
		if ((i_1_ & 0x20000) != 0) {
			...
			player.anInt10882 = Class174.anInt2092 + packet.readUnsignedShort();
		}
		if ((0x200 & i_1_) != 0) {
		...
		if ((0x4 & i_1_) != 0) {
			int i_20_ = packet.readUnsignedByte();
			if (i_20_ > 0) {
		...
		if ((i_1_ & 0x1000) != 0) {
			...
			player.anInt10883 = packet.readByte();
		...
		if ((0x2 & i_1_) != 0) {
			int i_35_ = packet.readUnsignedShort();
	...
	static final void method1035(int i) {
		...
		while (packet.method2264(Class218.worldResponseBuffer.anInt1581, -92) >= 15) {
