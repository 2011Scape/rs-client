...
public class Class289
{
	...
	private final void method3409(byte b, Buffer buffer, int i) {
		...
		if (i != 1) {
			if (i == 2) {
				anInt3640 = buffer.readUnsignedByte();
			} else if (i != 3) {
				if (i == 5) {
					aBoolean3636 = false;
				} else if (i != 7) {
					if (i == 8) {
						...
						if (i == 10) {
							...
							anInt3633 = buffer.readUnsignedByte();
						} else if (i == 12) {
							aBoolean3643 = true;
						} else if (i == 13) {
							anInt3646 = buffer.read24BitInt();
						} else if (i == 14) {
							anInt3634 = buffer.readUnsignedByte() << 2;
						} else if (i == 16) {
							anInt3641 = buffer.readUnsignedByte();
						}
					} else {
						anInt3632 = buffer.readUnsignedShort() << 2;
					}
				} else {
					anInt3629 = Node_Sub15_Sub12.method2582(buffer.read24BitInt(), -1);
				}
			} else {
				anInt3640 = buffer.readUnsignedShort();
				if (anInt3640 == 65535) {
					anInt3640 = -1;
				}
			}
		} else {
			anInt3631 = Node_Sub15_Sub12.method2582(buffer.read24BitInt(), -1);
		}
		int i_12_ = 6 % ((74 - b) / 49);
	}
