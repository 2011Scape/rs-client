...
public class Node_Sub38_Sub33 extends Node_Sub38 {
    ...
    static void method2898(int i, int i_0_, int i_1_, byte[] bs, int i_2_, byte[] bs_3_, int i_4_, int i_5_, int i_6_) {
        anInt10437++;
        int i_7_ = -(i_5_ >> 2);
        i_5_ = -(i_5_ & 0x3);
        if (i_6_ != 0) {
        ...
        for (int i_8_ = -i_4_; (~i_8_) > -1; i_8_++) {
            for (int i_9_ = i_7_; (~i_9_) > -1; i_9_++) {
                bs[i_1_++] += (byte) -bs_3_[i_2_++];
                bs[i_1_++] += (byte) -bs_3_[i_2_++];
                bs[i_1_++] += (byte) -bs_3_[i_2_++];
                bs[i_1_++] += (byte) -bs_3_[i_2_++];
            }
            for (int i_10_ = i_5_; (~i_10_) > -1; i_10_++)
                bs[i_1_++] += (byte) -bs_3_[i_2_++];
            i_1_ += i_0_;
            i_2_ += i;
        }
