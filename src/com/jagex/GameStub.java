...
public abstract class GameStub extends Applet implements Runnable, FocusListener, WindowListener {
    ...
    static void method83(byte b, int i) {
        anInt12++;
        CacheNode_Sub2 cachenode_sub2 = Class320_Sub19.method3754(3, 9, i);
        if (b > -43) {
            method101(-74);
        }
    ...
    private void method84(int i, boolean bool) {
        anInt26++;
        synchronized (this) {
    ...
    final boolean method95(byte b) {
        ...
        for (/**/; (~string.length()) < -1 && (~string.charAt(-1 + string.length())) <= -49; string = string.substring(0, string.length() - 1)) {
            if ((~string.charAt(string.length() - 1)) < -58) {
                break;
            }
        }
